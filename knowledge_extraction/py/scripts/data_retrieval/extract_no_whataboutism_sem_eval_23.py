...
directory_dev = 'C:\\Users\\lored\\OneDrive\\Desktop\\whatabout_research\\SemEval_23_en\\en\\dev-labels-subtask-3-spans'
articles_dev='C:\\Users\\lored\\OneDrive\\Desktop\\whatabout_research\\SemEval_23_en\\en\\dev-articles-subtask-3'
txt_files = []
label_files = []
text_data = []
...
def read_text_file(text_file_path, start_position, end_position):
    with open(text_file_path, 'rb') as text_file:
        text_file.seek(start_position)
        try:
            text = text_file.read((end_position+30) - (start_position-30)).decode('utf-8', errors='replace')
...
def main():
    ...
    # Read the line from the labels file
            ...
            for list in columns:
                        ...
                        if len(list) >= 2 and  "Whataboutism" not in list[1] :
                                if 'Red_Herring' not in list[1]:
...
with open("C:\\Users\\lored\\OneDrive\\Desktop\\whatabout_research\\txt\\500_extracted.txt", "w", encoding='utf-8') as output:
        unique= set(text_data)
        unique_l=list(unique)
        for t in unique_l[:501]:
